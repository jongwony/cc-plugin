...
- `snapshot --diff` compares node membership (by `backendDOMNodeId`), role, name, and nesting depth. It does **not** detect a pure sibling reorder that preserves all of those (same parent, same depth, same role and name — only order changed): such a change reports "no changes". A `--depth` change between calls IS detected and forces a full-tree re-baseline instead of a false diff. For order-sensitive verification, read the full tree (without `--diff`).
