...
## Running a Task
1. Run with the defaults — `k3[1m]` at `high` effort, thinking on — and choose a rung deliberately per the effort ladder below. The default carries the 1M window, so long-context work (multi-file refactors, very large scratchpad sessions) needs no flag.
