...
## Running a Task
1. Run with the defaults — `k3[1m]` at `high` effort, thinking on — and choose a rung deliberately per the effort ladder below. The default carries the 1M window, so long-context work (multi-file refactors, very large scratchpad sessions) needs no flag. When a run plainly does not need that window, `-m k3-256k` is the deliberate cheap option — see the quota note below, since the default is the more expensive of the two.

   **Model ids, per Kimi's documentation.** Two of them face Claude Code, and they are the only two the official setup block uses: `k3[1m]` (the default here — the 1M window) and `k3-256k` (K3 at a fixed 256K context). The default is **plan-gated at Allegretto or above**, so below that tier the default itself fails rather than degrading — a real narrowing versus a 256K default, which works from Moderato up. It is also the more expensive of the two: the docs put the 1M model at roughly twice the quota cost of `k3-256k`, which is why `-m k3-256k` is worth reaching for whenever the window is not the point. Plain `k3` is the 1M model under its canonical id; below Allegretto it is capped to 256K anyway, and the docs do not say whether such a capped call bills at the lower rate, so treat that as open rather than assuming a saving. Two more ids exist for the older generation — `kimi-for-coding` (K2.7 Code, available to every member, thinking always on with no effort granularity) and `kimi-for-coding-highspeed` (about 3× the quota for 5–6× output speed, Allegretto+) — worth knowing so a tier error reads as a tier error. The wrapper does not validate `-m`: unlike the effort set, the accepted ids are not a closed list, and `k3[1m]` is itself absent from the canonical four while appearing in the official setup block, so a validator would reject a value Kimi's own instructions hand you.
...
- A Kimi Code membership at **Allegretto or above for the default `k3[1m]`** — this is a gate on the default itself, not on an opt-in, so a Moderato membership must pass `-m k3-256k` explicitly to run at all. Verified accepted at the call level (2026-07); acceptance on its own leaves open whether a full 1M window was served.
- **Whether the 1M window is actually served remains unverified.** The wrapper now exports the pair Kimi's setup block ships alongside the model — `CLAUDE_CODE_AUTO_COMPACT_WINDOW` and `CLAUDE_CODE_MAX_CONTEXT_TOKENS`, both derived from the selected model rather than pinned — so the earlier gap of leaving them unset is closed. What is still unmeasured is the step after that: whether setting them delivers the full window against this endpoint. Nothing here upgrades that to verified. Our reading that `k3[1m]` is a Claude-Code-facing way to pin the 1M tier rather than a fifth backend model is **inference**: it appears in the official setup block but not in the canonical four-id list, and no doc states the equivalence.
...
- Cheaper, narrower run — `-m k3-256k` drops to the fixed 256K model at roughly half the quota, and works from Moderato up (the `k3[1m]` default needs Allegretto). Reach for it when the window is not the point
- Long context is the default — `k3[1m]` needs no flag; whether the full 1M window is actually served is still unverified, though the wrapper now sets the paired window variables. See Prerequisites before relying on it
