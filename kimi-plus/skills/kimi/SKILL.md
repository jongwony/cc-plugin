...
4. Delegate execution to a Bash subagent (Task tool) — never run `kimi-run.sh` directly in the main session. This keeps kimi's JSON response and full output out of the main context. Give the subagent:
   ...
   - liveness note: kimi runs are long — a build-scale task routinely runs 10+ minutes, past the foreground Bash timeout — so tell the subagent to invoke with `run_in_background: true` and let it run to exit. While it runs, stderr streams compact one-line `[kimi] <event>` markers (hook/init/thinking/tool/text/result activity) — expected, ordinary progress, not an error; the subagent should not relay these lines individually, nor read the long duration or the marker stream as a hang or failure. Only a non-zero exit, or a missing final `SESSION_ID:` line on stdout, means the run failed.
