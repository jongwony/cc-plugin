...
1. Run with the defaults — `k3[1m]` at `high` effort, thinking on. The default carries the 1M window, so long-context work (multi-file refactors, very large scratchpad sessions) needs no flag.
   ...
   **One model, one window.** This wrapper is built for a membership that carries the 1M tier, so `k3[1m]` is the default and the two context-window variables are pinned to match it rather than following `-m`. That premise is the single thing to revisit if the subscription ever changes. `-m` stays open as a passthrough — the wrapper does not validate it, because the accepted ids are not a closed list the way the effort levels are, and `k3[1m]` is itself absent from Kimi's canonical id list while appearing in their official setup block, so a validator would reject a value their own instructions hand you. The tradeoff of leaving it open: `-m k3-256k` drops to the fixed 256K model at roughly half the quota, but the window declaration stays at 1M. Reach for it when a run plainly does not need the window and the mismatch is acceptable.

   **Effort — `high` or `max`, and the axis is quota, not time.** `high` is the default and carries most frontend and artifact work. Pass `-r max` when the run turns on real design judgment rather than when a task merely feels important. Nothing here is floored by a clock — this wrapper runs in the background with no wall to beat — so what the choice costs is Kimi Code membership quota (see Quota Awareness), drawn from a pool shared with every other device and key.
...
One effort level per session, for the same cache reason — a change mid-session invalidates the prompt cache exactly as a model switch does, and the re-prefill is charged to quota. Pass the same `-r EFFORT` on every `-S` resume and track it in the {purpose → SESSION_ID} map beside the model.

## Quota Awareness

Kimi Code membership quota operates on a 7-day cycle counted from the subscription date — not a calendar week, and unused quota does not carry over — plus a rolling 5-hour window of roughly 300–1,200 requests at up to 30 concurrent. Every logged-in device and API key draws on the same pool, so a quota wall reached here was not necessarily spent here. On quota or 429-style errors, stop and report to the user — never retry-loop against a quota wall. Source: https://www.kimi.com/code/docs/en/kimi-code/membership.html

## Prerequisites

- A Kimi Code membership carrying the 1M tier. The wrapper assumes one and pins its window to match; a membership without it would need `-m` on every run and is not what this is built for. Verified accepted at the call level (2026-07).
- **Whether the 1M window is actually served remains unverified.** The wrapper exports the pair Kimi's setup block ships alongside the model — `CLAUDE_CODE_AUTO_COMPACT_WINDOW` and `CLAUDE_CODE_MAX_CONTEXT_TOKENS` — so the earlier gap of leaving them unset is closed. What is still unmeasured is the step after that: whether setting them delivers the full window against this endpoint. Nothing here upgrades that to verified. Our reading that `k3[1m]` is a Claude-Code-facing way to pin the 1M tier rather than a fifth backend model is **inference**: it appears in the official setup block but not in the canonical four-id list, and no doc states the equivalence.
...
- **Effort reaching the endpoint is inferred, not measured** (checked 2026-07): the basis is that Kimi ships `CLAUDE_CODE_EFFORT_LEVEL=high` in their own setup block, and no end-to-end run has confirmed the level changes the response.
...
- Long context is the default — `k3[1m]` needs no flag; whether the full 1M window is actually served is still unverified. See Prerequisites before relying on it
- Deeper reasoning — `-r max` (the default is `high`); an unaccepted value is rejected before the run rather than silently discarded. `kimi-run.sh -h` prints what the flag takes
