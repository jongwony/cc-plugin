...
## Error Handling
- **The wrapper does not intercept or reword failures.** On any nonzero exit — claude failing, a missing or locked gopass entry, a stream file that will not open, a malformed event log — the underlying tool's own stderr and exit code pass straight through (`set -e` aborts). That raw output is the diagnostic; for a claude or jq failure the full event log is in the stream file `<prompt>.stream.jsonl`. Read the raw failure — it is the ground truth — and fix over the next turn rather than expecting a crafted one-line explanation from the wrapper.
- **A successful (exit 0) run can still carry an empty result — check for it; the wrapper does not.** The script prints whatever the final `result` event held. If the stream carried no result event, or a result event with an error subtype and no `.result` (quota exhausted, max turns, cancellation), the RESULT and the `SESSION_ID` line come back empty even though the exit code is 0. Do not trust exit 0 alone: when the result is empty, open the stream file and read the final result event's `subtype`/`is_error` for the cause. The `SESSION_ID` may still be present (resume works) even when the answer is empty.
