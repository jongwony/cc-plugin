...
rather than from `tabs_create_mcp`, since the run is what caused it to exist.
Closing the group's last tab removes the group and its window; no separate
step. A tab the user moved in stays open.
